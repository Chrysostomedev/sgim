...
export default function StatsCard({ label, value, delta, trend = "up", isCurrency = false, href, shouldTruncate = true }: StatsCardProps) {
  ...
  const inner = (
    <>
      <p className="text-slate-900 text-[11px] font-semibold mb-4 px-1 leading-tight">{label}</p>
      <div className="flex items-center justify-between px-1 gap-2 min-w-0">
        {showFCFA ? (
          <div className="flex items-baseline gap-1.5 min-w-0 flex-1">
            ...
            <span className="text-[10px] font-bold text-slate-900 tracking-widest flex-shrink-0">FCFA</span>
          </div>
        ) : (
